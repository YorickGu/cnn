...
# Parameters settings
# Data loading params
tf.flags.DEFINE_string("database_path", "./AG/", "Path for the dataset to be used.")
...
def train_step(x_batch, y_batch):
    ...
    print("{}: Step {}, Epoch {}, Loss {:g}, Acc {:g}".format(time_str, step, int(step // num_batches_per_epoch) + 1,
                                                              loss, accuracy))
...
for train_batch in train_batches:
    ...
    if current_step % FLAGS.evaluate_every == 0:
        ...
        print("{}: Current Max Acc {:g} in Iteration {}".format(time_str, max(acc_list), int(
            acc_list.index(max(acc_list)) * FLAGS.evaluate_every)))
